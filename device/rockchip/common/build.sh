...
function usage()
{
	...
	echo "gentoo             -build gentoo rootfs"
...
function build_modules(){
   ...
   TARGET_ROOTFS_DIR="$TOP_DIR/$RK_ROOTFS_SYSTEM/binary"
...
function build_gentoo(){
	...
	echo "=========Start building gentoo for $ARCH========="
	...
	VERSION=debug ARCH=$ARCH ./mk-rootfs-gentoo.sh
   build_modules
	./mk-image.sh
...
function build_rootfs(){
	...
	case "$1" in
		...
		gentoo)
			build_gentoo
			ln -rsf gentoo/gentoo-rootfs.img \
				$RK_ROOTFS_DIR/rootfs.ext4
			;;
		*)
			build_buildroot
...
#RK_TARGET_PRODUCT defined in device/rockchip/.BoardConfig.mk is created by lunch
...
for option in ${OPTIONS}; do
	...
	case $option in
		...
		*.mk)
		...
		rootfs|buildroot|debian|gentoo|yocto) build_rootfs $option ;;
		pcba) build_pcba ;;
